class Solution:
    ...
    '''
    DFS: only process each node once. for eacch processed node we try to go in 4 directions. O(V+E): in this case V = mn. E = 4mn. thus this is in O(mn)
    Space: since depth of dfs is at most O(mn), and since each stack frame just stores i, j, loop vars, O(1) stuff, it is O(mn)
    '''

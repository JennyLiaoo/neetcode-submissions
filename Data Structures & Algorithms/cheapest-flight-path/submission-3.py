...
class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, k: int) -> int:
        ...
        Time complexity:
        O(k) outer loop, For each vertice consider its indegree. This will accumulate to considering every edge in the graph. This is overall O(kE)

        Auxiliary Space complexity:
        O(V+E) + O(VK)
        O(VK) -> O(V) since i only relies on i-1. order here is unpredictable since we could need any neighbor before/after airport in the array. so two lists
        '''
        ...
        min_cost_prev = [float('inf') for _ in range(n)] # 0 to k+1
            ...
            for airport in range(n):
                if airport == src:
                    ...
                    for neighbor, cost in adjlist[airport]:   # neighbor -> airport, airport -> [list of airports that fly TO it]
                        min_cost[airport] = min(min_cost[airport], min_cost_prev[neighbor] + cost)
            min_cost_prev, min_cost = min_cost, min_cost_prev

        if min_cost_prev[dst] == float('inf'):
            return -1
        return min_cost_prev[dst]

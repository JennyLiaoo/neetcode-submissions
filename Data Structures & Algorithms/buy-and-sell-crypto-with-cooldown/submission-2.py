class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        0 to i-2
        '''
        max_profit = [0 for _ in range(len(prices)+1)]
        max_profit[0] = 0
        for i in range(1, len(prices)+1):
            ...
            for j in range(i-1):
                if prices[j] < prices[i-1]:
                    if j >= 2:
                        max_profit[i] = max(max_profit[i], max_profit[j-1] + prices[i-1] - prices[j])
                    else:
                        max_profit[i] = max(max_profit[i], prices[i-1] - prices[j])
                        
        return max_profit[len(prices)]
